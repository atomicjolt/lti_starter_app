...
ActiveRecord::Schema[7.0].define(version: 2022_03_31_003859) do
  ...
  create_table "que_jobs", force: :cascade do |t|
    ...
    t.check_constraint "jsonb_typeof(args) = 'array'::text", name: "valid_args"
    t.check_constraint "jsonb_typeof(args) = 'array'::text", name: "valid_args"
    t.check_constraint "jsonb_typeof(data) = 'object'::text AND (NOT data ? 'tags'::text OR jsonb_typeof(data -> 'tags'::text) = 'array'::text AND jsonb_array_length(data -> 'tags'::text) <= 5 AND \"atomicjolt-helloworld\".que_validate_tags(data -> 'tags'::text))", name: "valid_data"
    t.check_constraint "jsonb_typeof(data) = 'object'::text AND (NOT data ? 'tags'::text OR jsonb_typeof(data -> 'tags'::text) = 'array'::text AND jsonb_array_length(data -> 'tags'::text) <= 5 AND admin.que_validate_tags(data -> 'tags'::text))", name: "valid_data"
    t.check_constraint "jsonb_typeof(data) = 'object'::text AND (NOT data ? 'tags'::text OR jsonb_typeof(data -> 'tags'::text) = 'array'::text AND jsonb_array_length(data -> 'tags'::text) <= 5 AND auth.que_validate_tags(data -> 'tags'::text))", name: "valid_data"
