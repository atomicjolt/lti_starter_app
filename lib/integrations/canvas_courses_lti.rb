  ...
  class CanvasCoursesLti < CanvasBaseLti
    ...
    def self.courses(canvas_authentication, lti_launch_url)
      ...
      courses.each do |course|
        if (course["enrollments"].map { |e| e["type"] } & allowed_enrollments).present?
          if find_tool_id(api.get_course_lti_tools(course["id"]), lti_launch_url)
    ...
    def self.setup(course, consumer_key, shared_secret, provider_url, token, lti_options)
      ...
      api = LMS::Canvas.new(UrlHelper.scheme_host(provider_url), token)
      ...
      if (id = find_tool_id(existing_tools, lti_options[:launch_url]))
        ...
        lti_connected_resource = ApplicationInstance.find_by(lti_key: tool["consumer_key"])
        if lti_connected_resource.blank?
          # The user or account that connected to the tool is no longer in the system or has changed
          # their LTI key. We need to update the key and secret.
          tool_config["consumer_key"] = consumer_key
          tool_config["shared_secret"] = shared_secret
        end
        # Important! If lti_connected_resource is valid then don't update the 'oauth_consumer_key'
        # or else external identifiers will break.
        api.proxy(
