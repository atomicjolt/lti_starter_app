    ...
    def self.setup(account, consumer_key, shared_secret, provider_url, token, lti_options)
      ...
      if (id = find_tool_id(existing_tools, lti_options[:launch_url]))
        ...
        if lti_connected_resource.blank?
          # The account that connected to the tool is no longer in the system
          # or has changed their LTI key. We need to update the key and secret.
          tool_config["consumer_key"] = consumer_key
          tool_config["shared_secret"] = shared_secret
        end
        # Important! If lti_connected_resource is valid then don't update
        # the 'oauth_consumer_key' or else external identifiers will break.
