...
export function Header(props) {
  ...
  const {
    ...
    paid_at: isPaid,
    free_trial_days_left: daysLeft,
    site,
    license_end_date: licenseEndDate
  } = applicationInstance;

  const endName = isPaid ? 'License Ends' : 'Trial Ends';
  const trialDays = daysLeft < 0 ? 'Expired' : `${daysLeft} Days`;
  const licenseDate = licenseEndDate ? moment(licenseEndDate).format('MM/DD/YY') : 'Never';
    ...
    <div className="aj-header">
      ...
      <div className="header-row info">
        <div className="paid">
          {isPaid ? (
            <>
              <i className="far fa-check aj-icon" />
              Paid
            </>
          ) : (
            'Trial'
        ...
        <div className="info-spacing">
          <div>{endName}</div>
          <div>{isPaid ? licenseDate : trialDays}</div>
        </div>
      </div>
    </div>
  );
